...
inline Intersection Triangle::getIntersection(Ray ray)
{
    ...
    float dis = 0.f;
    dis += std::pow(inter.coords.x - ray.origin.x, 2);
    dis += std::pow(inter.coords.y - ray.origin.y, 2);
    dis += std::pow(inter.coords.z - ray.origin.z, 2);

    inter.distance = std::sqrt(dis);
    return inter;
}
